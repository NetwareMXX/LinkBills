...
function Guardar() {
    ...
    $.ajax({
        type: 'GET',
        url: _encabezado + "/Costo/obtenerCostos?idMes=" + _idmes,
        dataType: 'json',
        beforeSend: function () {
            $("#tablaMenu").html("");
            $("#divProgreso").show();
...
function porSerie() {
    ...
    $.ajax({
        type: 'GET',
        url: _encabezado + "/Costo/obtenerFacturasSerie?IdMovimientosCostos=" + _recuperarCostos + "&TipoDePolizas=" + _tipo + "&IdTabla=" + _tabla,
        dataType: 'json',
        beforeSend: function () {
...
function porFactura() {
    ...
    $.ajax({
        type: 'GET',
        url: _encabezado + "/Costo/obtenerFacturasFactura?IdMovimientosCostos=" + _recuperarCostos + "&TipoDePolizas=" + _tipo + "&IdTabla=" + _tabla,
        dataType: 'json',
        beforeSend: function () {
            var window = myWindow.data("kendoWindow");
            window.element.prev().find(".k-window-title").html("<b class='custom'>Facturas Por Costo</b>");
...
function removerPorSerie(serie, factura) {
    ...
    $('.chkPer').each(function (index, obj) {
        ...
        if (this.checked === true) {
            ...
            $.ajax({
                type: 'GET',
                url: _encabezado + "/Costo/desligarCorrecion?idTabla=" + _tabla + "&movConsPol=" + _conspol.toString().trim() + "&movConsMov=" + _consmov.toString().trim() +
