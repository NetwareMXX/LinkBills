...
datosGrid = {
    ...
    IdCuentaCosto: { type: "string" }
};
_accionGrid = "/CuentaCostos/obtenerCuentas";
_accion = "/CuentaCostos/agregarCuentas/";
...
columnasGrid = [{
    command: [{ iconClass: "k-icon k-edit", name: "c", text: "", click: rowclick, title: "Editar", width: "30px" }
     , { iconClass: "k-icon k-delete", name: "d", text: "", click: borrar, title: "Desligar", width: "30px" }
                 ...
                 {
                     field: "IdCuentaCosto",
                     title: "IdCuentaCosto", hidden: true
                 },
                 {
                     field: "Nombre",
                     title: "Nombre"
...
function recuperarDatos() {
    _cuenta.IdCuentaCosto = $("#hdIdCuenta").val();
...
function llenarDatos(dataItem) {
    $("#hdIdCuenta").val(dataItem.IdCuentaCosto);
...
function borrar(e) {
    ...
    if (dataItem) {
        ...
        swal({
            title: "Esta Seguro de Eliminar la Cuenta Costo " + dataItem.Cuenta+" "+dataItem.Nombre + "?",
            ...
            closeOnConfirm: false
              ...
              function () {
                  ...
                  var url = _encabezado + "/CuentaCostos/eliminarTabla";
                  var datosAccion = {
                      "idTabla": dataItem.IdCuentaCosto
                  };
                  var self = this;
                  axios.post(url, datosAccion)
                          .then(function (response) {
                              if (response.data.Result == "OK") {
                                  swal("Cuenta Costo Eliminada!", "Se ha Removido exitosamente la Cuenta Costo", "success");
